# SPEC-B1 — THE PRODUCTION BACKEND
### Engineering specification for the Ash & Archive Studio production backend — auth, licensing, billing, sync relay, Press portal hosting, metered-AI proxy
*v1.1 · Status: **SEALED CANON — patched per adversarial verification 2026-07-12 (all defects resolved)** · Substrate: SPEC-001-FOUNDATION v1.1*
*Vendor ADRs (ADR-000–ADR-008) are **ratified-DEFERRED** by founder ruling 2026-07-12 (ADR-LOG) — provider-agnostic by design, not blocking. G-11 (Bind-while-lapsed) is **RESOLVED** by the same ruling: local Bind is never gated.*
...
Single logical region for v1 with the blob store CDN-fronted globally (read-only portals are static). **ADR-003** (cloud host) fixes the concrete provider; residency guarantees for EU users are deferred to a GAP (privacy law review is out of scope for this spec).
...
- **Access token** — short-lived JWT (TTL **15 min** — tuning value, G-10), signed by the token-signing key (KMS, ADR-004). Claims: `sub=accountId`, `dev=deviceId`, `ent=<entitlement digest>` (§3.4), `exp`, `iat`, `jti`. Stateless verification at the edge.
- **Refresh token** — opaque 256-bit random, **stored hashed** (SHA-256) in Postgres, rotating on every use (one-time-use; reuse detection → revoke device, §2.6). TTL **60 days** sliding (tuning value, G-10).

Flow:
```
POST /v1/auth/signup { email, credentialProof }      → 202 (account created 'active', emailVerifiedAt=null; verification email sent)
POST /v1/auth/verify-email { token }                 → 200 (sets emailVerifiedAt) | B-2004 EmailVerifyInvalid
POST /v1/auth/verify-email/resend { }                → 202 (rate-limited)
...
**Email verification gating:** an unverified email blocks only the routes that depend on email as a security factor — device pairing via email challenge (§2.3), account deletion (§2.7), and billing checkout (§4) — never signin itself and never anything local (BI-1). Verification-token TTL/format follows the ADR-001 provider where hosted; the endpoints above are the provider-neutral boundary.
...
POST /v1/devices/pair/initiate { newDevicePubSigKey, platform, displayName }
      → 200 { pairingCode, expiresAt }        // 8-digit, TTL 10 min, single-use (tuning values, G-10)
// On a trusted device:
...
**Device revocation** (lost/stolen/retired device):

```
POST /v1/devices/{deviceId}/revoke   → 204     // epoch-checked security-sensitive route (§2.4)
// Server: marks the device 'revoked', kills all its refresh tokens, bumps the account tokenEpoch
// so any outstanding access tokens fail on sensitive routes within ≤15 min everywhere else.
// Client chain (§5.5.1): a surviving trusted device bumps keyEpoch and re-wraps RCK to the
// remaining active devices' pubSigKeys; new sync blobs use the new epoch, so the revoked
// device can never decrypt anything written after revocation. Emits device.revoked (audited, §8.3).
```

Errors: `B-2101 PairingCodeInvalid`, `B-2102 PairingCodeExpired`, `B-2103 DeviceLimitReached` (see §3.2 tier `deviceLimit`), `B-2104 DeviceNotFound`.
...
```
active ──request──▶ pending_deletion ──(grace 30d — tuning value, G-10; cancelable)──▶ purge ──▶ deleted (tombstone)
                          │
                    cancel│ (any signin)
                          ▼
                        active
...
type EntitledState = Tier | 'unentitled';   // 'unentitled' = the covenant floor (below) — NOT a tier, never sold, never expires
...
**The covenant floor (`unentitled`).** There is **no free tier** — Ember (~$12/mo) is the *cheapest paid tier*, not a baseline anyone falls to. An account with no active subscription (never subscribed, lapsed beyond grace, canceled, dunning-exhausted, chargeback) holds the **covenant-floor state `unentitled`**: **full local rights** — open, read, author, Bind, play, and export every local world (LI-1; founder ruling 2026-07-12, ADR-LOG) — and **zero networked services** (no sync relay, no hosted portals, no metered AI). Lapse never routes to Ember; it routes here. The covenant floor is issued as a signed EntitlementDoc with `tier` field carrying `'unentitled'` semantics and no `notAfter` pressure on local rights.

**New-account default (no subscription yet):** a brand-new account holds the covenant floor. Whether signup includes an introductory trial (which tier, how long) is a business decision → **GAP (G-17)**; the engineering default is covenant floor.

### 3.2 The licensing invariant (constitutional)

> **LI-1 (never lock the user out of their own local data):** No tier, no lapse, no downgrade, no deletion may make a local Vault unreadable, un-editable, **un-Bindable**, or un-exportable *for the user's own worlds*. **Read, export, and local Binding of one's own ash are covenant-protected rights** (founder ruling 2026-07-12, ADR-LOG: "read and export never lock; the person's created world is never held hostage"). Licensing gates **networked services** (sync relay, hosted portals, metered cloud AI), never local authoring or access to data in the Vault. (BI-2; canon covenant; SPEC-001 I-1/I-6, §6, §9.)

Concretely: when a subscriber lapses, **local authoring and Binding continue in full on their own worlds** — a lapsed subscriber can still author canon in their own local world. Only the networked services gate (§3.5). Export is always full and lossless (SPEC-001 §9.2). (G-11 RESOLVED — ADR-LOG, Marcus rulings 2026-07-12.)
...
| Local authoring/**Binding** on the user's own worlds (all modules, incl. premium) | **Continues in full — never gated** (covenant floor, §3.1) | founder ruling 2026-07-12 (ADR-LOG); LI-1 |
| Networked services | Only these gate on lapse — the three rows below | covenant ruling: premium gates apply only to *networked* features |
...
**Confidence (this section): Medium-High.** Entitlement-as-signed-data, offline grace, and LI-1 degradation are strongly derivable; the covenant boundary (local Bind never gated) is founder-ratified (ADR-LOG 2026-07-12). Open: Ember sync inclusion (ADR-002, deferred) and several numeric limits (G-10).
...
- `CANCELED`/`UNPAID` (dunning exhausted) → EntitlementDoc reissued at the **covenant floor (`unentitled`, §3.1)** — never at Ember, which is a paid tier; local data and local Binding untouched (LI-1). Portals grace-then-suspend (§6.6).

### 4.5 Dunning / failure states

Dunning schedule is the **provider's** (do not reinvent). Backend consequence: keep entitlement at current tier through `PAST_DUE`; on final failure, downgrade to the covenant floor per §4.4.

**Composition rule — the two grace clocks never stack.** The provider dunning window (§4.4 `PAST_DUE`, server-side) and the offline grace window (§3.3 `graceDays`, client-side) run **concurrently, never sequentially**. The client's effective horizon is always `notAfter + graceDays` of the **last signed EntitlementDoc it holds** — dunning only determines *which* doc the server issues next (full tier during `PAST_DUE`; covenant floor after exhaustion). A client that is offline through both windows therefore degrades at its own `notAfter + graceDays`; it does not get the dunning window *added on top*, and a connected client whose dunning exhausts drops to floor on its next refresh even if `graceDays` remain unelapsed locally (the fresher floor doc supersedes).

### 4.6 Refund / downgrade / cancellation

- **Cancel** → `cancelAtPeriodEnd=true`; features persist to period end (§3.6 downgrade rules).
- **Refund** → provider action via adapter; refund does **not** retroactively delete authored data (LI-1). If a refund implies immediate service loss (fraud/chargeback), entitlement drops to the covenant floor (§3.1) immediately; **local data still untouched**, export still available.
...
|---|---|
| `accountId`, `deviceId`, `worldId` (as an opaque ULID — no name), `deviceSeq`, `lamport`, `blobId`, `vocabVersion`, byte length, `contentType='ash.event'|'entry.version'|'snapshot'|'attachment'`, wrapped-key blobs (opaque), timestamps of relay | event `type`, payload, entry bodies, names, prose, canon status content, disclosures, veiled content, attachment bytes — **all AEAD ciphertext** |
...
interface SyncEnvelope {
  ...
  vocabVersion: number;            // SPEC-001 §3.2 VOCAB_VERSION — cleartext metadata; enables the soft
                                   // peer-ahead warning (§9.1) without payload inspection (BI-3)
...
*All numeric budgets in this table are tuning values (G-10) — targets, not canon-derived constants.*
...
| Range | Family | Codes |
|---|---|---|
| B-20xx/21xx/22xx | Auth/devices/deletion | `B-2001 SigninFailed` · `B-2002 RefreshInvalid` · `B-2003 RefreshReuseDetected`(→revoke) · `B-2004 EmailVerifyInvalid` · `B-2101 PairingCodeInvalid` · `B-2102 PairingCodeExpired` · `B-2103 DeviceLimitReached` · `B-2104 DeviceNotFound` · `B-2201 DeletionAlreadyPending` · `B-2202 NothingToDelete` |
...
1. **Covenant survival (BI-2, LI-1):** with every backend service down AND the account deleted, a client can still open, edit, **Bind (always — never gated; founder ruling 2026-07-12, ADR-LOG)**, and export every local world losslessly (SPEC-001 §9.2 round-trip). *Test: kill all services + delete account → full local loop passes, including Binding new ash.*
...
Each: Context · Options · Recommendation · Consequences · What would change it. **None is chosen by fiat.**

> **Founder ruling 2026-07-12 (ADR-LOG): ADR-000 through ADR-008 are ratified-DEFERRED.** "Don't decide vendors before a running Foundation — premature commitment." The backend stays provider-agnostic; the local product builds fully without any vendor chosen. These reopen when the Foundation runs. **Not blocking.** (Per the same log, ADR-B1-004's *shape* — primitives and recovery model as specified in §5.5 — is designed here; only the vendor/KMS choice defers.)
...
### Builder Friction Index (BFI): **80 / 100** *(verifier re-score post-patch)*

*Interpretation:* a Builder agent could mechanically implement the **structural spine** — schemas, API contracts, state machines, the E2E relay shape, the LOCKED-only publish guard, the AI-proxy ingress guard, error taxonomy, degradation matrices, and the now-ratified covenant boundary (local Bind never gated; covenant-floor `unentitled` state) — without further design. It **cannot** reach 100 because the vendor ADRs (ratified-DEFERRED by founder ruling — reopen when the Foundation runs) and the business-policy gaps below are genuine decisions that must be made by the founder, not invented (per METHOD RULES: ZERO INVENTION). The invariants and the client↔server contracts are High-confidence and implementable today; the vendor-and-policy layer is deliberately deferred rather than fabricated.

### GAP REGISTER (everything below 100, what's missing, why)

| # | Gap | What's missing | Status / why open (not invented) |
|---|---|---|---|
| G-1 | Billing vendor | concrete provider + mobile-store strategy | ADR-000 — **ratified-DEFERRED** (founder ruling 2026-07-12); reopens when the Foundation runs |
| G-2 | Auth provider | hosted vs. self-run | ADR-001 — **ratified-DEFERRED** (same ruling) |
| G-3 | Ember sync inclusion | tier boundary for sync | ADR-002 — **ratified-DEFERRED** (pricing; same ruling) |
| G-4 | Cloud host/CDN/object store | concrete infra | ADR-003 — **ratified-DEFERRED** (same ruling) |
| G-5 | KMS vendor + recovery-model choice | passphrase vs. device-only recovery; concrete KMS | ADR-004 — vendor **ratified-DEFERRED**; the crypto *shape* (§5.5 primitives, epoch/re-wrap chain) is sealed here |
| G-6 | Atelier team seats | seat model under BI-5 | ADR-005 — **ratified-DEFERRED** (GTM vs. v2.0 timing) |
| G-7 | Entitlement push channel | pull vs. push | ADR-006 — **ratified-DEFERRED** (latency tolerance) |
| G-8 | Reader auth for gated cohorts | Patreon OAuth vs. reader accounts | ADR-007 — **ratified-DEFERRED** (creator platforms) |
| G-9 | LLM provider(s) + zero-retention terms | provider + contracts | ADR-008 — **ratified-DEFERRED** (procurement) |
| G-10 | Numeric limits & tuning constants | `worlds.max` fair-use, `press.portals.max`, `deviceLimit` exacts, `graceDays` value, `maxTokens`/allotments per tier, token TTLs (15 min / 60 d, §2.2), pairing code format/TTL (8-digit / 10 min, §2.3), deletion grace (30 d, §2.7), sync SLO budgets (§5.9) | tuning + "final numbers at beta" (01-VISION §IV) — not fiat-able |
| G-11 | ~~"Bind while lapsed?" edge~~ | **RESOLVED** — founder ruling 2026-07-12 (ADR-LOG): **local Bind is never gated**; read, export, and local Binding of one's own ash are covenant-protected rights; premium gates apply only to networked features (sync, hosted portals, cloud AI). LI-1 extended accordingly (§3.2, §3.5, §12 AC#1). | CLOSED |
| G-12 | AI overage policy | overage hard-stop vs. bill-per-unit | business policy (dunning/offline-grace composition is now specified — §4.5 composition rule) |
...
| G-15 | Data residency / EU | region guarantees, GDPR/CCPA specifics | legal review out of scope for this spec |
| G-16 | Cert-pin rotation ops | operational runbook for pin rotation | minor ops detail |
| G-17 | Introductory trial policy | whether new accounts get a trial tier, and for how long (engineering default: covenant floor, §3.1) | business decision |

*Prepared under the ZERO-INVENTION method and patched per adversarial verification 2026-07-12 (verdicts C1, H1, M1–M5, L1–L4 — all resolved). Every decision herein either cites SPEC-001 v1.1 / STUDIO-GENESIS / the ecosystem canon / the ADR-LOG rulings, or is raised as an ADR/GAP. Nothing is silently chosen. This document is subordinate to SPEC-001; on any conflict, SPEC-001 and the ecosystem canon govern and this document must be corrected.*
