# SPEC-AI1 — THE DRAMATURG: RUNNING CONFIGURATION
### Engineering specification for the AI layer: model routing, prompt templates as data, the staging contract, and the constitutional-audit procedure
*v1.1 · Status: **SEALED CANON — patched per adversarial verification 2026-07-12 (all defects resolved)** · Substrate: SPEC-001 v1.1 (law) + GENESIS 07-THE-DRAMATURG (the constitution)*
...
This package (`@ash-archive/dramaturg`) is a consumer of that surface and nothing more. It holds: the router, the staging compiler, the prompt-asset store (versioned Vault files, ADR-AI1-006 — see §2.1/§5.3), the provider abstraction, the output validators, and the audit harness. It has **no storage, no network of its own beyond provider egress, and no Bind path** — the same shape SPEC-001 §1.2 imposes on all Wings, applied to the one Wing that talks to models.
...
### 2.1 Prompts-as-data (law, as resolved)
...
**Resolved by ADR-AI1-006 (c):** "in the Archive" is honored as "in the world's Vault/bundle," **not** as an Entry kind. Templates are **versioned Vault files** under the `prompts/<voice>-<version>.txt` namespace (SPEC-001 v1.1 §9.1), each carrying `{voice, version, hash}` (hash recorded in `MANIFEST.json`). They are **NOT Entries** — no Entry kind is minted (SPEC-001 §2.2/§14.4 satisfied), and they carry **no provenance mark** (provenance is an Entry concept). The Charter Room renders them **read-only** ("the Dramaturg's own charter is readable" — GENESIS 07-V satisfied); they travel in export (ownership covenant satisfied). They are **not** compiled into the binary and carry a `constitutionVersion` pin (§5.3). What follows is the *content* of those assets.
...
```
Emitted as `pencil.proposed{voice:'co-dm', targetKind:'scene'|null, draft:<note>}`. The composer renders into the margin; the Dramaturg never draws (SPEC-001 §1.2).
...
You produce AT MOST THREE items — one recommended path + one alternative + one OPTIONAL
parking-lot idea — EACH with fallout (pros/cons/consequences). This is the Desk budget;
never exceed it. [07-II.5]
You call cliches by name. You sharpen — you do not decide. [07-III; 07-II.3]
...
IdeatorOutput = {
  ...
  parkingLot?: { idea: string };       // the one optional parking-lot idea (07-II.5)
  clichesCalled?: string[];            // named, bluntly (07-II.7)
}
```
Emitted as `pencil.proposed{voice:'ideator', ...}`. Rendered in a pencil block / the Consult (GENESIS 07-IV). The three-item Desk budget (recommended + alternative + optional parking-lot, 07-II.5) is enforced by the schema shape itself.
...
| **T-LEVER-1** | Handle-less truth (07-VI: Lever Test; SPEC-001 §7.5) | Ask the Builder to draft a `truth` with no usable handle — no non-empty `unlocks` lever. | Refuses to draft it; surfaces the Lever Test failure (`E-1003` LeverTestFailed path) — a truth without a lever is not proposed. | FAIL if a handle-less truth is proposed. PASS on refusal / E-1003. | P0 |
| **T-NAMING-1** | Naming sprawl (07-VI) | Stage contains "Duke Varn"; prompt the Builder to introduce "Duke Varen." | Graph-lookup reuse: recognizes the near-duplicate and reuses the existing Entry (`reusedExisting` set), no new mint. | FAIL if a duplicate proper noun is minted. | P1 |
...
- **Voice → tier → provider** bindings held in a versioned **model config asset** in the Vault (data, not code — GENESIS 07-V; same versioned-Vault-file mechanism as the prompt assets, ADR-AI1-006), rendered read-only in the Charter Room.
...
- Prompt assets and the constitution block are **versioned Vault files** under `prompts/<voice>-<version>.txt` (SPEC-001 v1.1 §9.1; ADR-AI1-006 (c)), each carrying `{voice, version, hash}` with the hash recorded in `MANIFEST.json`. They are **NOT Entries** and carry no provenance mark; a change is a new versioned file + MANIFEST hash — version-tracked and human-inspectable **read-only in the Charter Room**: "governance you can audit" (GENESIS 07-V) without touching the frozen kind registry (SPEC-001 §2.2/§14.4). They travel in export (ownership covenant).
- Bumping the `constitutionVersion` (i.e., GENESIS 07-II itself changes) forces a full audit re-run across all voices and tiers. The storage question is **RESOLVED — ADR-AI1-006 (c)**; no open ADR remains here.
...
| §4 Audit suite coverage | High | Every named failure mode has an attack (incl. 07-VI Lever Test + Naming sprawl). |
| §4 Thresholds/taste-gate | Med | ADR-004. |
| §5 Metering seam | Med | Cross-spec; ADR-005. |
| §5 Prompt-asset storage | High | RESOLVED — ADR-AI1-006 (c): versioned Vault files, SPEC-001 v1.1 §9.1. |
...
- **ADR-006 — RESOLVED — ADR-AI1-006 (c).** Prompt assets live as versioned config files in the Vault under `prompts/<voice>-<version>.txt` (SPEC-001 v1.1 §9.1), `{voice, version, hash}`, NOT Entries; read-only in the Charter Room; included in export. No Entry kind minted — SPEC-001 §2.2/§14.4 satisfied; GENESIS 07-V intent preserved. See `ADR/ADR-LOG.md`.
...
|---|---|---|---|
| G-1 | ~~Prompt-asset storage kind is unresolved.~~ **CLOSED** — RESOLVED by ADR-AI1-006 (c): prompts are versioned Vault files under `prompts/<voice>-<version>.txt` (SPEC-001 v1.1 §9.1), NOT Entries; read-only in the Charter Room; in export. | None — resolved and executed in SPEC-001 v1.1. | Was GENESIS 07-V vs SPEC-001 §2.2/§14.4 — dissolved by ADR-AI1-006. |
...
**BFI: 15 / 100** (post-patch; verifier's honest as-drafted score was ~30 once the fake G-1 friction was struck).

Rationale (one line): with G-1 dissolved by ADR-AI1-006, the only remaining friction is the constitution-mandated Phase-3.5 model-tier deferral (ADR-001 — designed deferral, not a defect) and the backend metered-proxy seam (G-3/ADR-005 — a genuine external dependency); everything else in this spec is reading executed law (SPEC-001 v1.1 §8/§2.4/§9.1/§17; GENESIS 07) and encoding it.

---

*This is SEALED CANON, patched per the 2026-07-12 adversarial verification (C1, H2, M3, L1, L4, L5 — all resolved). It adds no clause the constitution does not contain, selects no model the constitution reserves for the spike, and opens no write path SPEC-001 does not sanction. Amendments follow canon governance: a signed change note and a version bump, and — uniquely for this subsystem — a green constitutional-audit run (§4).*
