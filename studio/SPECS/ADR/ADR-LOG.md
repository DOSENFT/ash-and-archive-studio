...
## CORE §19 STEP 3 BUILT (2026-07-16) — §5 API + query builder
- `packages/core`: archive surface complete per §5.2/§5.3 (get/query/history/links/subgraph/search/draft/reviseDraft/link/endLink/disclose/archiveEntry), EntryQuery builder (§5.5 methods exactly; SQL never crosses the API line), §5.7 RiteSet registry, perspective redaction below the API line (E-1101; undisclosed never stages). **Tests 20 → 35 green (verified independently); strict tsc clean.** In-test §15 sanity: get/query/links p99 ≤3ms, search p95 ≤100ms on seeded fixtures; the 100k harness remains step 7.
- **Twelve non-dictated decisions recorded in the build report (none silent)** — notable for the founder/spec: (1) **E-1102 StaleHead is unimplementable from the dictated signature** — `reviseDraft(id, body, actor)` carries no expected-base-version param; detecting concurrent revision requires a SPEC-001 additive amendment (v1.3 candidate); (2) `EntryHead.name` sourced from `body.name` (spec silent); (3) contentless-FTS re-index quirk documented (stale terms may match an entry; aliases legitimately should per §7.3); (4) §5.7 opaque types deferred to SPEC-R1's shapes. Deferred-by-spec: E-1003 at lock/plan (v1.2 law), UNKNOWN_STATUS_FIELDS at transitions (§6/§7), metrics/contradictions/veiled-ash-window (later steps).
- Next in the lane: **step 4 — §6 Binding.**

## THE MOTION DOCTRINE & LANTERNLIGHT GRADE CODIFIED (Marcus instrument, 2026-07-16 — SH3 §11.9)
- **LOCK-V1..V4 appended to POSITIVE-LOCKS.md, retroactive** (short clips ≈3–5m / ≤30° facing; mirrored pose pairs are cut-pairs, never dolly-pairs — warp exhibit `a4af6038` retained; style never enters the video model — texture-preservation preamble + motion/physics only; chain joints from encoded-clip frames, vendor presets declined with `declined_preset_id`).
- **Lanternlight Grade v1 checked in** (`studio/PIPELINE/grade/lanternlight-v1.sh`) — the single source of the film stock, applied pre-intake; seam checks on graded output; grade pinned in the manifest (`lanternlight-grade-v1`); grade change = ADR = full re-render by design. HARVEST-INTAKE gains §C-bis.
- **Flight production plan ratified**: chained short dives per the route table, texture-preamble prompts, graded, seam-gated on extracted endpoints, intake-gated. Reference exemplar dive `8c0061aa` registered as SR-0025 (RMS 9.76 vs canon Pose A). Stills-floor remains shipping; flights purely additive.
- **Ledger addendum registered**: six bay stills incoming with generation IDs (Chronicle e0ea5cc9 · Forge 34652505 · Charter ae4baf13 · Stage fd9a6ff3 · Academy 7a97107c · Press db5c7eca) as SR-0019..SR-0024, receive-wired; completes all eight bays + garth + gate + approach on receipt.

## CORE §19 STEP 4 BUILT (2026-07-16) — §6 The Binding
- `packages/core/src/binding/`: `plan()` pure+deterministic w/ planHash · `commit()` single-transaction (banked/full; `binding.ratified`+`binding.sealed` inside the tx — no partial Binding can exist) · §7.4 contradiction detectors (all three) with machine `contradicts`-link docketing · E-1003 lever law at plan AND commit · E-1301 idempotency returning the original receipt reconstructed from ash · §7.1 status machine incl. UNKNOWN_STATUS_FIELDS enforcement · chaos kill-mid-commit test (integrity clean, ash gapless, replay commits). **Tests 35 → 49 green (verified independently); strict tsc clean. `binding.plan` @400-event session: 3.2–6.7ms (law 1,500ms).**
- **Twelve non-dictated decisions recorded in the build report, none silent** — notable for the founder: (4) a brand-new truth from a pencil proposal can never bind in the same ceremony (its unlocks link cannot pre-exist E-1003's plan-time test) — it holds as ash; consistent with ADR-003-D's lock/plan timing but worth the founder's eyes as a felt-at-the-table consequence; (1) `ruling.made`/`inscription.added` draft no upserts (visible in scene groups only); (6) banked mode emits both ratified+sealed so planHash idempotency stays detectable.
- Next in the lane: **step 5 — §7 Charter.**

## CORE §19 STEP 5 BUILT (2026-07-16) — §7 Charter
- `packages/core/src/charter/`: lock/demote/docket/resolve/readiness/rulings per §7 — E-1003 at lock incl. lever-broken (ADR-003-D honored: mark, never a save rejection at draft) · §7.4 detection on lock docketing as machine `contradicts` links · PatchChoice minimal/clean/story executed transactionally with §7.3 alias protocol · v1.2 ReadinessReport shape + readiness-visibility rule + thresholds-as-data via Ruling entry. **Tests 49 → 76 green (verified independently); strict tsc clean.**
- **Sixteen non-dictated decisions recorded, none silent.** For the founder / SPEC-001 v1.3 amendment register: (1) **no `charter.*` events exist in the sealed 68-type vocabulary** — charter acts create EntryVersions and append no ash (minting a type would violate §14.5); (5) **the Charter is necessarily a third `entry_versions` write path** vs §4.2's "exactly two" — v1.3 should enumerate it; (12) **SPEC-001 §7.5 ("beings/things") and SPEC-003 §2 (toy row includes `place`) disagree** on toy-counted kinds — SPEC-001 taken as law for core pending ruling; (14) region-scope membership undefined — implemented as 1-hop active-link neighborhood, amendment candidate; (11) readiness `borderline` quantified as total shortfall ≤2 (deterministic, tunable).
- Next in the lane: **step 6 — §9 export/import.**

## CB1 GATE 1 CLOSED (2026-07-16) — SPEC-CB1 v0.1 → v0.2
- Three fresh-context hostiles (fatigue · prosecutor · taste). **28 findings: 28 accepted, 0 rejected.** Verbatim transcript + dispositions: `drafts/CB1-GATE1-TRANSCRIPT.md`.
- Headline convictions, all cured in v0.2: the ≤16ms echo had no mechanism — input-priority scheduling + recompose-yields-to-buffered-input now law with a colliding fixture (F-1); no focus/caret law across recompose — recompose may never move focus, collapse a focused Quill, or discard uncommitted text (F-2); auto-turn under an active caret downgrades to offer (F-3); delta-burst coalescing law minted (F-4); **the 80ms budget was double-spent twice** — §8.1 summed to 84ms and the mount path to 125ms; both restated coherently against the named clock (C-1/C-2: 1+15+64=80; active folio first, neighbors post-`ready`); `pencil.dismissed` restored to the allowlist, `inscription.struck` struck as a dead second door (final count 12); **the combat-end exhale convicted as an unlicensed ceremony** and demoted to Transition 520ms — the GENESIS 03-VI strict list holds (T-1); the Bind absence re-voiced (`THE BINDING AWAITS`, once per world) and slot-exempt furniture law minted (T-2/C-11/F-7); ADR-CB1-E minted (search field desk-time only) + the field authored (§3.7).
- **Awaiting Marcus: seal ruling on SPEC-CB1 v0.2** (ADR-CB1-C flagged canon-adjacent for his eyes: Bind visibly absent at α; six-verb completeness gates ship, not slice).

## CORE §19 STEP 6 BUILT (2026-07-16) — §9 export/import (the ownership covenant made real)
- `packages/core/src/vault/{format,exporter,importer}.ts`: the §9.1 tree verbatim (WORLD.md, entries/<kind>/, history/, chronicle/, ash/events.jsonl, attachments/, prompts/, MANIFEST.json), human-readable frontmatter dialect with zero new dependencies (§17's cap honored), staged ImportPlan + transactional apply with per-item SAVEPOINTs, §9.2 edit-tolerance (hand-edited LOCKED head demotes to provisional — never silently canon). **Tests 76 → 86 green (verified independently); strict tsc clean. Export 463–539ms / import ~850ms at ~1/20 scale (law 30s).** Lamport now implements §3.1 literally (max-observed+1) — required once imported worlds carry foreign-device events; bit-identical for local logs.
- **Eighteen non-dictated decisions recorded, none silent.** SPEC-001 v1.3 amendment candidates from this step: (5) **§9.2 byte-identity and §9.3's `import.completed` marker are mutually exclusive as written** — implemented: the sole permitted delta is exactly one trailing marker line + its MANIFEST reflection; the spec should say where the marker lives; (6) **V0 import has no field-level payload schema anywhere in the sealed corpus** (GENESIS 08-IV fixes collections only) — `{kind:'v0'}` returns a clear failure pending a schema fixture or amendment. Deferred by jurisdiction: `prompts/` content storage (SPEC-AI1/ADR-AI1-006 territory — namespace exports empty until it lands); §12 metrics with step 7.
- Operational note, recorded: the save-all timer auto-checkpointed mid-build (`1ce9534`) — harmless under branch discipline (spec branch, seal-merge only), the final green state supersedes it.
- Next in the lane: **step 7 — §15/§16 full harnesses (the last rung).**

## ADR-SH1-SCROLL · Scroll is a first-class Passage input that scrubs flight time (EXECUTED BY CANON HOLDER'S HAND, 2026-07-17)
- **Ruling (Marcus, verbatim intent):** SH1 §2.6 omitted scroll; overruled by ADR. The scroll wheel / trackpad scrubs flight time — the camera genuinely moves, scroll only drives time, per the ingested scrub-engine doctrine (blob-seek, frame-accurate, no autoplay dependence). Discrete inputs coexist: click plays at tier speed; scroll scrubs under the hand. Reduced-motion unchanged. **Sealed physics untouched:** airlock, dormancy, decay, skip ≤120ms; scrub fully released ⇒ the engine settles to the nearest endpoint and seats.
- The shell's spine becomes THE GRAND TOUR: one continuous connected flight through the Cloister; where a connector clip does not yet exist the chain bridges with the lawful drift-cut/dissolve — continuous today, upgrading clip-by-clip with zero rework. Incoming connectors (provenance external-mcp-sprint): Approach→Gate `74731d65` · Gate→AmbulatoryA `5ef7b236` · AmbulatoryA push `8c0061aa` (present) · Corridor→Garth, Corridor→Codex to follow; each graded via lanternlight-v1 and seam-checked before mounting.
- **The veil convicted (same instrument):** the flat dark backdrop overlay desaturated canon imagery — struck. Full-fidelity artwork; page brightness by its own paper luminance and placement; any A-2 conflict solved with LOCAL gradient vignetting at the page card, never a global veil; on-screen color verified by sampled ΔE vs source stills.
- **Harness law (same instrument):** self-verification owns its own lifecycle — launch, capture, terminate by PID with a hard timeout; a human closing windows is a harness defect.

## /GOAL — THE ONE BUILD (founder directive 2026-07-17; suspensions logged one line each, reconciliation is the founder's later problem)
- Suspended in the goal build: the rail + SH1 §2.6 discrete-nav contract during the journey (scroll only; skip-on-input kept) · bay travel/drift-cuts/palette/ring-walk (no bays exist on screen) · ThrowawayFolio mounting (file retained; CB1 unaffected) · the Gate-1 T-3 chip spec (amended to 12px corner; law's teeth — ship-lint — untouched) · dev contract monitor.
- Delivered: launch → continuous scroll journey (moor → gate flight `74731d65` → threshold flight `5ef7b236` → corridor scrub `8c0061aa` → garth → Codex) → seated at a working desk; words save to disk via the Rust host (`codex/first-page.md`, atomic write) and survive relaunch. Engine = the vendored fork, verbatim. **Scrub-law re-encode applied to all three clips (crf20/g8/faststart) — the grade script's default GOP made backward scrub unusable; provenance hashes updated; the encode step is now permanently part of receiving flights.** ffmpeg installed (runbook debt closed). Goal acceptance 9/9 incl. relaunch persistence; SR-0033/34 queued for intake.

## RESOLVED by Marcus (2026-07-19 instrument — THE EMBODIMENT CAMPAIGN bound as active charter)
*The full ruling is bound at `studio/SPECS/THE-EMBODIMENT-CHARTER.md` (sealed by the instrument itself). Four campaign ADRs + one parking order minted:*

- **ADR-EMB-A · The design-production layer is promoted to the lead workstream.** The ladder's standing clause ("out of scope for this campaign… a separate final campaign *after* behavior is sealed") is superseded by promotion for the **sealed tier only**: Foundation (SPEC-001 v1.2) · Composer (SPEC-002 v1.1; the seated instrument still gates on the CB1 seal, SHIP-LEDGER row 7) · World Forge (SPEC-003 v1.1) · Codex (GENESIS v2.0). SPEC-004/005/006 stay outside scope until sealed through the normal pipeline — the campaign invents nothing. **Reverses if:** never expected; descoping would be a new instrument.
- **ADR-EMB-B · Zero Behavioral Drift is immutable project law.** No inventing, merging, simplifying, renaming, extending, or reinterpreting product behavior. Improvement lives exclusively in presentation, interaction, visual communication, motion, discoverability, ergonomics, accessibility, responsiveness, implementation quality, craftsmanship. Apparent behavioral necessity → `EMBODIMENT-DRIFT-DOCKET.md` (append-only, opened this date), never implementation. **Reverses if:** never — drift proposals route through the docket to the canon holder.
- **ADR-EMB-C · SPEC-SH2 (`lanternlight-v1`) is the visual constitution for the entire platform**, not only the world layer. Higgsfield expresses SH2 across every software surface and visual artifact; it creates no new visual language. All standing generation discipline applies unchanged (shot records, HARVEST-INTAKE, UNCURATED law, spend gate before any credit burns). Scroll World, when it ships, must read as the inevitable spatial extension of this interface. **Reverses if:** a `lanternlight-v2` is minted by ADR — which stales pinned surfaces by construction, as already law.
- **ADR-EMB-D · The platform anchor is explicit and constitutional for the campaign:** production-grade desktop-first Tauri, React as the presentation layer, aligned with the existing Rust host architecture. No SaaS assumptions, no browser-first compromises, no placeholder patterns. **Reverses if:** a platform ADR with Marcus's signature — same bar as every constitutional ceiling.
- **ADR-SH4-PARK · SPEC-SH4 is PARKED, unsealed — not killed.** The draft, WORLD-BIBLE-V1, STYLE-BLOCK-V3, and PROMPTS-V2 stand as authored; no adversarial verify, no seal, no spend-tier approval proceeds while parked (SHIP-LEDGER row 15 annotated). Reopens by Marcus's instrument only, as the spatial extension of a shipped embodied interface. **Reverses if:** Marcus reopens it — that is the design.
