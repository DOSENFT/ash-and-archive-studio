...
### 11.9 THE MOTION DOCTRINE & THE LANTERNLIGHT GRADE (founder instrument, 2026-07-16 — sealed pipeline law, entered by ruling)
- **LOCK-V1 · Short clips only:** no generated clip traverses a full route or changes facing >30°; max displacement ≈3–5m of level dolly; long journeys are chains of short clips, never single takes. The route table's segments are therefore themselves chains where their length demands it.
- **LOCK-V2 · Mirrored pose pairs are cut-pairs, not dolly-pairs:** travel between them is chained dives joined on extracted frames, or the drift-cut — never one clip. Cautionary exhibit: archived warp artifact `a4af6038`, retained.
- **LOCK-V3 · Style never enters the video model:** video prompts carry the texture-preservation preamble — *"the world does not re-render; only the viewpoint moves"* — plus motion and physics only. All style language lives in the stills and in the post-grade. (The §11.4 template's STYLE section is amended accordingly for video: the Canon Style Block belongs to still generation and to the grade, not to motion prompts.)
- **LOCK-V4 · Chain joints from encoded clips:** joint frames are extracted from ENCODED clips via ffmpeg, never source stills; vendor preset auto-substitution is always declined, `declined_preset_id` recorded whenever offered.
- **The Lanternlight Grade v1** (`studio/PIPELINE/grade/lanternlight-v1.sh`) is the single source of the film stock — halation bloom, Kodak-curve lift, desaturation, grain, vignette — applied identically to every clip **before intake**; **seam checks run on graded output**. Grade version is manifest metadata (`grade: "lanternlight-grade-v1"`); a grade change is an ADR and re-renders the whole library by design — determinism is the point.
- **The validated flight method** (ratified): the flight library is chained short dives per the route table — ring segments ≈3–5m each, bay dive-ins from ambulatory poses, gate-cross, approach push — each generated with texture-preamble physics prompts, graded via lanternlight-v1, seam-gated on extracted endpoint frames, intake-gated by the canon holder. 1080p std minimum. Reference exemplar: dive `8c0061aa` (SR-0025 — 5s, 1920×1080, first-frame RMS 9.76 vs canon Pose A). Stills-floor remains the shipping configuration; flights remain purely additive.
